...
@pytest.fixture
def ids_file(client):
    """The /api/book tests' mapping file, removed afterwards: `client` is module-scoped."""
    path = client.data_dir / "trainline_ids.json"
    yield path
    path.unlink(missing_ok=True)


def test_book_redirects_matched_pair_to_results(client, capsys, ids_file):
    stations = client.get("/api/stations").json()["stations"]
    a, b = stations[0]["id"], stations[1]["id"]
    ids_file.write_text(json.dumps({a: "7630", b: "7480"}))
    ...
    assert f"BOOK from={a!r} to={b!r} date='2099-01-05' matched=true" in capsys.readouterr().out


def test_book_log_escapes_newlines(client, capsys):
    client.get("/api/book", params={"from": "a\nBOOK forged", "to": "b", "date": "x"},
               follow_redirects=False)
    out = capsys.readouterr().out
    assert "\nBOOK forged" not in out
    assert "from='a\\nBOOK forged'" in out


def test_book_unmatched_goes_to_homepage(client, ids_file):
    ids_file.write_text("{}")
    resp = client.get("/api/book", params={"from": "a", "to": "b", "date": "2099-01-05"},
                      follow_redirects=False)
    assert resp.status_code == 302
...
def test_book_without_mapping_file_goes_to_homepage(client, ids_file):
...
def test_book_applies_link_prefix(client, monkeypatch, ids_file):
