...
| Norway | Entur national aggregated GTFS | UNFIT (verified 2026-07-14) | Official stable anonymous 610 MB GTFS under NLOD 2.0, with coordinates and hierarchy. F1/F4/F5/F6/F7 are narrow rail route labels, but replacement buses are mixed into those routes without a usable trip-level mode (F5 includes bus-terminal stops), so the current pipeline would map buses as trains. Revisit for a rail-only GTFS or documented replacement-bus marker. |
